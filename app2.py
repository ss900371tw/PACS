...
# ====== 移除 Unsloth 模型載入，改由 MONAI Deploy MAP 處理 ======
print("系統初始化：已切換為 MONAI Deploy MAP 推理模式。")
...
# ====== MONAI Deploy MAP 設定 ======
# 根據你的環境，如果是從容器內呼叫，請確保 localhost 或 IP 能對接到 Orthanc 服務
MAP_IMAGE_NAME = "simple_app-x64-workstation-dgpu-linux-amd64:latest"
...
def analyze_image_with_monai_map(instance_id):
    """利用 MONAI Deploy CLI 呼叫 MAP 進行 VLM 報告預測"""
    try:
        # 1. 組合 Orthanc 的圖片預覽網址 (對外/對容器可存取的完整 URL)
        # 注意：如果 MONAI Deploy 在獨立容器或主機運行，請確保 localhost 能正確導向 Orthanc
        preview_url = f"{ORTHANC_URL}/instances/{instance_id}/preview"

        # 2. 構建 CLI 執行指令
        cmd = f"monai-deploy run {MAP_IMAGE_NAME} -i {preview_url}"
        print(f"[MAP 執行指令]: {cmd}")

        # 3. 透過 subprocess 呼叫系統指令並獲取標準輸出
        # 使用 shlex.split 安全解析帶有字串參數的指令
        result = subprocess.run(
            shlex.split(cmd),
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            text=True,
            timeout=60  # 設定 60 秒超時防卡死
        )

        if result.returncode != 0:
            print(f"MONAI Deploy 執行失敗，錯誤訊息:\n{result.stderr}")
            return "Analysis Failed (MAP CLI Error)"

        # 4. 取得 MAP 輸出的報告結果
        # 提示：如果你的 MAP 輸出包含日誌，你可能需要根據 MAP 的 stdout 格式進行 .split() 或正則過濾
        diagnosis = result.stdout.strip()
        
        # 預防輸出為空字串的防錯處理
        if not diagnosis:
            diagnosis = "Normal / No specific findings detected by MAP"

        print(f"[MONAI MAP 診斷結果 - ID: {instance_id}]: {diagnosis}")
        return diagnosis

    except subprocess.TimeoutExpired:
        print(f"MONAI Deploy 執行超時 (60s): {instance_id}")
        return "Analysis Failed (Timeout)"
    except Exception as e:
        print(f"MONAI MAP 呼叫過程中發生錯誤: {e}")
        return "Analysis Failed (MAP Integration Error)"
...
def update_instance_tag_keep_same_id(instance_id, comment_text):
    ...
    try:
        ...
        # 在刪除與重新上傳前，預先將該 SOPInstanceUID 寫入已處理快取
...
def process_webhook_task(sop_id):
    ...
    try:
        ...
        # 呼叫 MONAI Deploy MAP 替代原本的本地端 Unsloth
        diagnosis_result = analyze_image_with_monai_map(instance_uuid)
